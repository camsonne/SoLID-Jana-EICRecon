...
def plot_vs_lumi(runs, resdir):
    ...
    for i, ro in enumerate(readouts):
        ...
        eff = [x["summary"].get("efficiency_vs_geometric_acceptance", x["summary"]["reco_efficiency_in_acceptance"]) for x in rs]
    ...
    axes[1].set_ylabel("e', μ⁺, μ⁻ all reconstructed / all within geometric acceptance", color=TEXT, fontsize=9)
...
def write_table(runs, resdir):
    ...
    for r in runs:
        ...
        lines.append(
            f"| {r['L']:.2g} | {r['readout']} | {s['n_events']} | {s.get('geometric_acceptance', float('nan')):.3f} | {s.get('efficiency_vs_geometric_acceptance', float('nan')):.3f} | {s['reco_efficiency_in_acceptance']:.3f} | "
